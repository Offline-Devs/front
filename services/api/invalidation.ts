...
export const invalidation = {
  ...
  adminStudent: (id: string) => [["admin", "students"], queryKeys.adminStudent(id), ["admin", "students", id, "statistics"]] as QueryKey[],
  blog: [queryKeys.blog, ["admin", "blog"]],
  dynamicFields: [["admin", "dynamic-fields"]],
} satisfies Record<string, QueryKey[] | ((id: string) => QueryKey[])>;

export async function invalidateDependencies(client: QueryClient, keys: QueryKey[]) { await Promise.all(keys.map((queryKey) => client.invalidateQueries({ queryKey }))); }
