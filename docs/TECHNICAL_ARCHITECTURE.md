...
### سامانه اعلان‌های سراسری

اعلان‌ها با **Sonner** و یک `Toaster` واحد در بالاترین provider برنامه نمایش داده می‌شوند. محل نمایش
در بالای مرکز صفحه ثابت است و عرض، فاصله، آیکن، دکمه بستن، سایه و انیمیشن ورود برای موبایل و دسکتاپ
تنظیم شده‌اند. رنگ سطح، متن، حاشیه و وضعیت‌های موفق، خطا، هشدار و اطلاع‌رسانی از design tokenهای
برنامه استفاده می‌کنند؛ بنابراین بدون نگهداری دو stylesheet مستقل، هم‌زمان با تم روشن و تیره تغییر
می‌کنند.

`MutationCache` و `QueryCache` در `lib/query-client.ts` نقطه کنترل سراسری اعلان‌های شبکه هستند. خطای
تمام mutationها و خطای بارگذاری اولیه queryها از همین مسیر گزارش می‌شود. پیام موفقیت هر mutation نیز
در `meta.successMessage` همان mutation تعریف می‌شود تا متن هر عملیات مشخص باشد و اعلان تکراری در
component ایجاد نشود. خطای refresh پس‌زمینه وقتی داده معتبر در cache وجود دارد به‌صورت toast مزاحم
نمایش داده نمی‌شود.

اعتبارسنجی فرم‌ها از `lib/form-notifications.ts` استفاده می‌کند. این helper نخستین خطای تو‌در‌توی
React Hook Form یا خطای اعتبارسنجی runtime را پیدا می‌کند و با شناسه ثابت نمایش می‌دهد تا چند خطای
هم‌زمان به چند اعلان تکراری تبدیل نشوند. عملیات محلی مانند تغییر تم، اعمال یا پاک‌کردن فیلتر، خروج
از حساب و کنترل فایل نیز اعلان متناسب دارند.
...
BFF فقط headerهای پاسخ موردنیاز را عبور می‌دهد. headerهای `Retry-After` و `X-RateLimit-*` بک‌اند به
`ApiError` تبدیل می‌شوند تا شمارش‌گر ارسال مجدد OTP زمان واقعی اعلام‌شده توسط سرور را رعایت کند.
