...
پاسخ‌های ناموفق به نمونه `ApiError` تبدیل می‌شوند. این کلاس status، کد پایدار، قابلیت retry، زمان
مجاز تلاش مجدد و metadata محدودیت نرخ را از headerهای `Retry-After` و `X-RateLimit-*` نگهداری
می‌کند. همچنین پیام کنترل‌شده فارسی را نگهداری می‌کند. کامپوننت‌ها وضعیت‌های خطا، retry، empty و
permission را به‌صورت یکپارچه نمایش می‌دهند. متن خام خطای بک‌اند هیچ‌گاه مستقیماً وارد HTML نمی‌شود.

payloadهای تاریخ در فرانت فقط با فیلد جلالی ارسال می‌شوند؛ بنابراین قرارداد بک‌اند که ارسال هم‌زمان
تاریخ میلادی و جلالی را رد می‌کند در سطح type نیز enforce شده است. تاریخ canonical برگشتی با قالب
صفرگذاری‌شده در mapperها مصرف می‌شود.
