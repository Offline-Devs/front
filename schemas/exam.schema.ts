...
export const subjectExamSchema = z.object({
  ...
  if (value.correct + value.wrong !== value.answered) {
    context.addIssue({ code: "custom", path: ["answered"], message: "پاسخ‌داده‌شده باید برابر صحیح + غلط باشد." });
  }
  if (value.answered + value.blank !== value.total_questions) {
    context.addIssue({ code: "custom", path: ["total_questions"], message: "مجموع پاسخ‌داده‌شده و نزده با تعداد سؤال برابر نیست." });
  }
});

export const examSchema = z.object({
  title: z.string().trim().min(2, "عنوان آزمون را وارد کنید."),
  jalali_date: z.string().transform((value) => normalizeNumericInput(value).replace(/-/g, "/")).refine(
  ...
  if (new Set(names).size !== names.length) {
    context.addIssue({ code: "custom", path: ["subjects"], message: "هر درس فقط یک بار قابل ثبت است." });
  }
});
