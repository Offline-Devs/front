...
export function sanitizeTelemetry(input: unknown): TelemetryEvent | null {
  ...
  if (typeof source.name !== "string" || !source.name.trim() || source.name.length > 80) return null;
  ...
  for (const key of ["value", "rating", "route", "digest"] as const) {
    ...
    if (key === "rating" && ["good", "needs-improvement", "poor"].includes(String(value))) clean.rating = value;
    if ((key === "route" || key === "digest") && typeof value === "string") clean[key] = value.slice(0, 160).split("?")[0];
  }
