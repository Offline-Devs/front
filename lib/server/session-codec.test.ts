...
describe("encrypted session codec", () => {
  it("round-trips a valid session without exposing token text", async () => {
    ...
    expect(await decodeSession(encoded, secret)).toEqual(session);
  });
  it("rejects a modified or wrongly signed cookie", async () => {
    ...
    expect(await decodeSession(encoded, "another-secret-with-more-than-32-characters")).toBeNull();
  });
});
