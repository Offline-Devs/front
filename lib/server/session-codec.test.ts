...
describe("encrypted session codec", () => {
  ...
  it("rejects a modified or wrongly signed cookie", async () => { const encoded = await encodeSession(session, secret, 60); expect(await decodeSession(`${encoded.slice(0, -1)}x`, secret)).toBeNull(); expect(await decodeSession(encoded, "another-secret-with-more-than-32-characters")).toBeNull(); });
});
