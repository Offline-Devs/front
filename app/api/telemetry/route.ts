...
export async function POST(request: Request) {
  ...
  const event = sanitizeTelemetry(JSON.parse(raw).catch?.(() => null) ?? (() => { try { return JSON.parse(raw); } catch { return null; } })());
  ...
  process.stdout.write(`${JSON.stringify({ level: event.type === "error" ? "error" : "info", event, at: new Date().toISOString() })}\n`);
