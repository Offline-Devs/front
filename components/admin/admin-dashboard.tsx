...
export function AdminDashboard() {
  const all = useQuery({
    ...
    queryFn: () => adminApi.students({ page: 1, limit: 1, approved: "all" }),
    staleTime: 30_000,
  });
  const approved = useQuery({
    queryKey: ["admin", "dashboard", "approved"],
    queryFn: () => adminApi.students({ page: 1, limit: 1, approved: true }),
    staleTime: 30_000,
  });
  const pending = useQuery({
    queryKey: ["admin", "dashboard", "pending"],
    queryFn: () => adminApi.students({ page: 1, limit: 1, approved: false }),
    staleTime: 30_000,
  });
  if (all.isLoading || approved.isLoading || pending.isLoading)
    return (
  ...
  if (all.isError) return <ApiErrorState error={all.error} retry={() => void all.refetch()} />;
  const items = [
    { label: "کل دانش‌آموزان", value: all.data?.total ?? 0, icon: UsersRound },
    { label: "تأییدشده", value: approved.data?.total ?? 0, icon: CheckCircle2 },
    { label: "در انتظار تأیید", value: pending.data?.total ?? 0, icon: Clock3 },
  ];
  return (
    <div className="grid gap-6">
      ...
      <Button asChild className="w-fit">
        <Link href="/admin/students?approved=false&page=1">بررسی پرونده‌های در انتظار</Link>
      </Button>
    </div>
  );
}
