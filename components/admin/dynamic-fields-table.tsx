...
export function DynamicFieldsTable() {
  ...
  const [editing, setEditing] = useState<DynamicFieldDefinition | null>(null);
  const fields = useQuery({
    queryKey: queryKeys.dynamicFields(entity === "all" ? undefined : entity),
  ...
  return (
    <div className="grid gap-5">
      <div className="flex items-center gap-3">
        <span className="text-sm font-bold">موجودیت:</span>
        <Select value={entity} onValueChange={setEntity}>
          ...
          <SelectContent>
            ...
            <SelectItem value="student">پروفایل</SelectItem>
      ...
      {editing && (
        <Card>
          <CardHeader>
            <CardTitle>ویرایش {editing.label}</CardTitle>
          </CardHeader>
          <CardContent>
            <DynamicFieldForm
              field={editing}
      ...
      {fields.isLoading ? (
        ...
        <ApiErrorState error={fields.error} retry={() => void fields.refetch()} />
      ) : fields.data?.length ? (
              ...
              <CardHeader>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <code className="mt-2 block text-xs text-muted-foreground" dir="ltr">
                      {field.name}
                    </code>
                  </div>
                  <Badge variant="outline">
                    {entityLabels[field.entity_type] ?? field.entity_type}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent className="flex items-center justify-between gap-3">
                <p className="text-sm text-muted-foreground">
                  نوع: {typeLabels[field.field_type]} {field.is_required ? "· الزامی" : "· اختیاری"}
                </p>
                <div className="flex">
                  <Button
                    size="icon"
                  ...
                  <ConfirmDialog
                    ...
                    description="تعریف فیلد حذف می‌شود؛ مقادیر قدیمی داخل رکوردها خودکار پاک نمی‌شوند."
        ...
        <EmptyState title="فیلد سفارشی تعریف نشده است" />
      )}
    </div>
  );
}
