...
export function MistakeForm({
  ...
  const form = useForm<MistakeFormValues, unknown, MistakeFormOutput>({
    ...
    defaultValues: {
      exam_id: mistake?.exam_id ?? "",
      subject_exam_id: mistake?.subject_exam_id ?? "",
      question_number: mistake?.question_number ?? 1,
      category: mistake?.category ?? "",
      notes: mistake?.notes ?? "",
      dynamic_fields: mistake?.dynamic_fields ?? {},
    },
  });
  ...
  const dynamicFields = useQuery({
    queryKey: ["dynamic-fields", "mistake"],
  ...
  const save = useMutation({
    ...
    onSuccess: async () => {
      await invalidateDependencies(queryClient, invalidation.mistake);
      onSaved?.();
      if (!mistake) {
    ...
    <form className="grid gap-5" noValidate onSubmit={form.handleSubmit(submit, notifyFormErrors)}>
      <div className="grid gap-4 sm:grid-cols-2">
        <FormField
          label="آزمون مرتبط"
          hint="اختیاری"
          error={form.formState.errors.exam_id?.message}
        >
          <Select
            value={examId || "none"}
            onValueChange={(value) => {
              form.setValue("exam_id", value === "none" ? "" : value);
              form.setValue("subject_exam_id", "");
            }}
          >
            <SelectTrigger>
              <SelectValue placeholder="بدون آزمون" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="none">بدون آزمون</SelectItem>
              {exams.data?.map((exam) => (
                <SelectItem key={exam.id} value={exam.id}>
                  {exam.title} — {exam.jalali_date}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <FormField
          label="درس مرتبط"
          hint="اختیاری"
          error={form.formState.errors.subject_exam_id?.message}
        >
          <Select
            value={subjectExamId || "none"}
            disabled={!selectedExam}
            onValueChange={(value) =>
              form.setValue("subject_exam_id", value === "none" ? "" : value)
            }
          >
            <SelectTrigger>
        ...
        <FormField
          label="شماره سؤال"
          error={form.formState.errors.question_number?.message}
          required
        >
          <Input
            type="number"
        ...
        <FormField
          ...
          hint="مثلاً بی‌دقتی یا ضعف مفهومی"
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" loading={save.isPending}>
          ...
          {mistake ? "ذخیره ویرایش" : "ثبت اشتباه"}
        </Button>
      </div>
    </form>
  );
